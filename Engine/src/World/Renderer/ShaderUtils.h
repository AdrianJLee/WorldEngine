...
namespace World
{
	...
	//
	// 引擎着色器(`Engine/assets/shaders/**`)是**Slang 单源**形态(`.slang`;HLSL 语法是
	// Slang 的子集),用组合采样器
	...
	{
	public:
		// 编译或加载指定的 Slang 源(`.slang`),返回**当前后端**的 SPIR-V 字节码
		...
		struct BakeResult
		{
			size_t Shaders = 0;    // 处理的 Slang 源文件数(`.slang`)
			size_t Artifacts = 0;  // 写出的产物数(SPIR-V)
			size_t Failed = 0;
			std::string Error;     // 首个错误(便于打包失败时报出)
		};
		// 兼容入口(EditorCooker 的 4a 步):把目录下全部 Slang 源(`.slang`)
		// 的 VSMain/PSMain 烘成
		...
		// Slang-T5:发行形态的**双目标**烘焙。对每个 Slang 源(`.slang`)
		// 把运行时真的会请求的入口
