		...
		AssetFingerprint FingerprintAssetInternal(const std::string& logicalPath, std::string* error, int depth)
		{
			...
			if (ReadAssetBytes(logicalPath, bytes, &readError))
			{
				...
				if (IsMaterialPath(logicalPath) && depth < kMaxMaterialParentDepth)
				{
					...
					// M4-S3:本文件写的 `Shader:` 引用的 `.hlsl` 内容也进指纹 —— 改代码 →
					// 引用它的材质指纹变化(缓存/热重载口径与父级链同款)。
					// 继承来的 shader 由父级那一层的指纹负责(递归里已经混进去)。
					if (document.HasShader && !document.Values.ShaderPath.empty())
					{
						const AssetFingerprint shader =
							FingerprintAssetInternal(document.Values.ShaderPath, nullptr, depth + 1);
						MixFingerprint(result.Value, shader.Value);
						MixFingerprint(result.Value, shader.Exists ? 0x9e3779b97f4a7c15ull : 0ull);
					}
				}
				if (error) error->clear();
				return result;
