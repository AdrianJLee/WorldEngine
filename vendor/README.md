...
本目录与 `Engine/vendor/`(原 `World/vendor/`)的划分**只看"交付形态"**,不看"谁在用"。任何第三方条目按三选一落位:

| 形态 | 放哪 | 入库策略 |
| --- | --- | --- |
| **参与编译的源码**(静态/动态库源码) | `Engine/vendor/<name>/`(现状;理想名 `third_party/<name>/`) | submodule 优先(仓库只存 commit);不能 submodule 的才 vendored 源码,必须写清版本 |
...
1. **一份内容只有一个位置,一个名字只指一个东西。** 反例(现存):`vendor/SPIRV-Cross/` 是 **CLI**,
   `Engine/vendor/SPIRV-Cross/` 是 **库**(submodule,`SPIRV_CROSS_CLI=OFF` + C API,被测试与
...
| `vendor/SPIRV-Cross/` | 工具 | `spirv-cross.exe` + LICENSE(≈2MB) | 运行期 SPIR-V → GLSL;库形态见 `Engine/vendor/SPIRV-Cross` |
...
- `bin/`、`bin-int/` 是**构建产物**规则(挡住 in-source 构建产物,例如 `Engine/vendor/Glad/bin/*.lib`)。
